...
> You are given an inclusive range [lower, upper] and a sorted unique integer array nums, where all elements are in the inclusive range.<br/>
A number x is considered missing if x is in the range [lower, upper] and x is not in nums.<br/>
Return the smallest sorted list of ranges that cover every missing number exactly. That is, no element of nums is in any of the ranges, and each missing number is in one of the ranges.<br/>
...
1. All the values of nums are unique.

### <AnchorLink label="Expected" name />
1. Sum of Two String return in String

### <AnchorLink label="Test Cases" name />
1. Input = nums = [0,1,3,50,75], lower = 0, upper = 99 Output = ["2","4->49","51->74","76->99"]<br/>
Explanation: The ranges are:<br/>
[2,2] --> "2"<br/>
[4,49] --> "4->49"<br/>
[51,74] --> "51->74"<br/>
[76,99] --> "76->99"<br/>
1. Input = nums = [-1], lower = -1, upper = -1 Output = []<br/>
...
1. Step 1: Initialize the array variable `result`.<br/>
1. Step 2: Initialize the integer variable `previous` with computing lower minus one (lower - 1).<br/>
1. Step 3: Iterate the given number `nums` array.<br/>
1. Step 3.1: Inside Loop, Initialize the integer variable `current` with value current indexed number `nums[i]` if its not last item else store the upper value with adding one (1).<br/>
1. Step 3.2: If `lower` value which is `previous` is still lesser than or equal to `current` value then read the `previous` + 1 (lower value) and `current` - 1 (upper value) to generate those series using `formatMissingNumbers`.<br/>
1. Step 3.2: Create `formatMissingNumbers` function to find all the missing numbers passsing previous plus 1 (`previous` + 1) and current minus 1 (`current` - 1).<br/>
1. Step 3.3: Push the Formatted value to `result` array.<br/>
1. Step 3.3: Assign the `current` value to `previous` variable.<br/>
1. Step 4: Return the result.<br/>
1. Step 5: In `formatMissingNumbers` function, read the lower and upper value.<br/>
1. Step 6: Return the lower if lower and upper are same.<br/>
1. Step 7: Else, format the number like `${lower} -> ${upper}`.<br/>
